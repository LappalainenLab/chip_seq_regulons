...
data = pd.read_table("data/regulons/TF_target_mapping_filtered_merged_K562_with_ppi_with_dnase_with_atac_with_motifs_with_ccres_cleaned.tsv", sep="\t")
...
s2mb_rand = dc.shuffle_net(s2mb, target='target', weight=None, seed=27).drop_duplicates(['source', 'target'])
m2kb_rand =  dc.shuffle_net(m2kb, target='target', weight=None, seed=27).drop_duplicates(['source', 'target'])
s2kb_rand =  dc.shuffle_net(s2kb, target='target', weight=None, seed=27).drop_duplicates(['source', 'target'])
